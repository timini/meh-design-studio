...
def bound_reference(path, project):
    payload = path.read_bytes()
    expected = project['physical_system'].get('metadata', {}).get('analytic_reference_sha256')
    if not expected or hashlib.sha256(payload).hexdigest() != expected:
        raise ValueError('analytic reference differs from the evaluated fixture identity')
...
def main():
    ...
    reference = bound_reference(args.fixture / "reference.json", project)
    ...
    if validate_electrical_basis(project_path, args.evaluation) != consistency or bound_reference(args.fixture / "reference.json", project) != reference:
        raise ValueError("tube comparison artifacts changed during calculation")
