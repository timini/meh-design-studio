"""Explicit FP64 runner using the pinned external Boundary Lab API.

Run with Boundary Lab's Python environment. Production CLI solves use FP32 for
BEM; this exposes the separate reference backend without relabelling FP32 output.
This file also runs directly under Boundary Lab's isolated Python environment.
"""
import argparse
...
def main():
    ...
    parser.add_argument("--julia-threads", type=int, default=4)
    args = parser.parse_args()
    if not 1 <= args.julia_threads <= 64:
        raise ValueError('Julia thread count must be in [1, 64]')
    ...
    execute_reference(writer, args.output, {"revision":revision, **runtime_identity},
        ...
        lambda: CoupledReferenceBackend(julia_executable=args.julia, julia_threads=args.julia_threads,
