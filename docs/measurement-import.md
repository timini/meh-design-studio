...
Numeric uncertainty and coordinate declarations require JSON numbers; booleans and numeric strings are rejected. Derived floating-point payloads are compared bit for bit, preserving the sign of zero as part of phase evidence. Unsupported ZIP compression and encrypted/unreadable archive failures produce structured validation errors rather than uncaught CLI tracebacks.

Only regular files are read: Unix artifacts are opened nonblocking and checked with `fstat` before data reads, preventing a substituted FIFO from hanging inspection. Python 3.14 Zstandard decompression failures are also normalized as invalid archives; older Python versions do not require that optional standard-library module.

Archive resource limits: the reader requires exactly four central-directory entries, a single disk, no archive comment or ZIP64 directory, and at most 4096 directory bytes. It walks those four records before constructing ZipFile, rejecting forged counts and trailing entries. NumPy local ZIP64 size headers remain supported. Stored and DEFLATE members are accepted; BZIP2, LZMA, Zstandard and other compression methods are rejected before opening a decompressor because output limits are not consistently enforced across supported runtimes.

Manifests are limited to 4096 bytes before JSON decoding; larger raw evidence files retain the separate 16 MiB cap. Derived floating-point arrays use explicit little-endian float64 on every host for portable byte-exact verification (the raw CSV remains unchanged). Rejected compression formats require no optional decoder imports.

Metadata declarations are limited to 128 KiB before parsing or validation on both import and inspection. JSON declarations reject duplicate keys at every object level, including repeated phase or unit fields; preserved raw bytes must therefore have an unambiguous interpretation. Manifest schema_version must be an integer equal to 1, excluding booleans and floating-point values.
