...
After reservation, POSIX publication holds a directory descriptor and creates files and renames the manifest relative to that descriptor. Replacing the output pathname cannot redirect those writes. Windows holds a native directory handle without delete sharing until publication finishes, preventing directory rename/replacement; reparse-point handles are rejected. The Windows lock regression must run on Windows before this behavior is claimed validated there.
