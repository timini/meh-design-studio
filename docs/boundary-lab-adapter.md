...
A real coupled run exposed an intermittent macOS process-group cleanup error after all frequencies had finished; its evaluation stayed failed. Darwin cleanup now checks an EPERM against the whole process group and accepts only an empty group or zombie-only members. Any live member preserves the error. Parent cleanup still runs if group cleanup fails. The native behaviour is consistent with [Darwin process-group signalling](https://github.com/apple-oss-distributions/xnu/blob/main/bsd/kern/kern_sig.c); tests distinguish empty/zombie groups from a live child.

The termination guard is installed before a durable running report is written. Completed evaluations also retain `preflight_sha256`; consumers must recheck that file before accepting the saved compiled contract. FEM result topology is compared with independently loaded source tetrahedra selected by the project volume tags, including node compaction and all ten quadratic nodes. Unsupported/mixed volume elements are rejected. Darwin exiting groups receive a bounded reaping grace; a live group that remains inaccessible still fails.

Output reservation defers SIGINT (and POSIX SIGTERM) until ownership of the new directory is known, then records cancellation inside the reporting handler. Existing output directories are never adopted or rewritten. Once a terminal status is selected, cancellation signals are ignored until its atomic report is committed; a completed solve therefore takes precedence over cancellation arriving during final publication. Previous handlers are restored afterward. The adapter must run in the main thread of its isolated worker on all platforms. Tests inject both POSIX signals during directory creation and completed/failed publication, including repeated signals. BEM triangle arrays must use integer indices, just like FEM connectivity.

Process creation also defers cancellation until the new child PID is inside its cleanup handler, including an unassigned suspended Windows child. Unknown output identifiers and non-object JSON artifacts are rejected as structured validation errors.

The pinned runtime requires exactly one FEM mesh per bounded region, and this adapter requires one owning bounded region per FEM mesh. Multiple separate bounded regions remain supported. This is an explicit supported-domain limit: the [pinned upstream result writer rejects multi-mesh bounded regions](https://github.com/JWSound/boundary-lab/blob/8cb166226e412877d3f71f2845918e479b97aa85/src/blab/solve_results/fem.py#L45). A field's membership check alone does not establish broader runtime support. Selected volume tags containing non-tetrahedral cells are rejected; unsupported cells outside the selected volume do not enter the FEM result domain.
