...
[Explicit curved mid diaphragms](curved-mid-diaphragms.md) now let the source
surface and front cavity follow an axisymmetric Bezier profile. Full and quarter
meshes distinguish actual surface area from the projected moving area. Rear air
...
[Absolute material tessellation](material-export-tolerance.md) fixes the STL
tolerance being interpreted relative to edge size. STL and 3MF now share one
checked mesh, including removal of collapsed empty pole facets. Export topology
and the unchanged 1% CAD-volume limit are checked before solver preparation.
The concentric pilot's original failed export and all native inputs are retained.
